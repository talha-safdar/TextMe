    ...
    <ul class="pagination justify-content-center paginationChange">
        ...
        if($_SERVER['REQUEST_URI']) // if it has the URI to access the page
        {
            ...
            if (isset($_SESSION['user_ID'])) // if user is logged in
            {
                ...
                if ($pageNo > 1) // if is greater than 1
                {
                    ...
                    echo '<a class="page-link" href="friends.php?page=' . $lower . '"' . 'tabindex="-1">Previous</a> ';
                }
                else
                {
                    // keep the same variable with the current value if is not more than 1
                    echo '<a class="page-link" href="friends.php?page=' . $pageNo . '"' . 'tabindex="-1">Previous</a>';
                }
            } ?>
        </li>
            <?php
            if(isset($_SESSION['user_ID'])) // if user is logged in
            {
                if(isset($_SESSION['pageNumbersFriends'])) // if global variable with page numbers is set
                {
                    for($i=1; $i<=$_SESSION['pageNumbersFriends']; $i++) // loop from to the maximum number of pages
                    {
                        if ($i == (int) $pageNo) // if $i equals to $pageNo 
                        {
                            // display the matching value
                            echo '<li class="page-item shadowPagMid"><a class="page-link aPageItems" href="friends.php?page=' . $i . '">' . $i . '</a></li>';
                        }
                        else if ($i < (int) $pageNo && $i != $_SESSION['pageNumbersFriends']) // if $pageNo is greater than iterated value and not equal to pageNumbers
                        {
                            echo ""; // if condition met do not show right sided numbers
                        }
                        elseif ($i > (int) $pageNo && $i != $_SESSION['pageNumbersFriends']) // if $pageNo is lesser than iterated value and not equal to pageNumbers
                        {
                        ...
                        else
                        {
                            ...
                            echo '<li class="page-item shadowPagMid"><a class="page-link" href="friends.php?page=' . $i . '">' . $i . '</a></li>';
                        }
                    }
                }
            } ?>
        ...
        <li class="page-item shadowPagNext">
            ...
            if(isset($_SESSION['user_ID'])) // if user is logged in
            {
                ...
                if ($pageNo < $_SESSION['pageNumbersFriends']) // is $pageNo is less than page numbers
                {
                    // use $higher variable to increase to go the next page
                    echo '<a class="page-link" href="friends.php?page=' . $higher . '"' . 'tabindex="-1">Next</a> ';
                } 
                else 
                {
                    // keep the same variable with the current value if is not less than page number
                    echo '<a class="page-link" href="friends.php?page=' . $pageNo . '"' . 'tabindex="-1">Next</a>';
                }
            }
        } ?>
        </li>
