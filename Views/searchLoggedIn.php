...
<div id="loading" class="loading">
    ...
    <div class="container containerTable">
        ...
        <h3 class="col-lg-4 searchMatch">Matches returned</h3>
        </div>
                    ...
                    <div class="card-body text-center">
                        ...
                        <div class="text-center">
                            ...
                            <form action="../Models/Core.php" method="post">
                                ...
                                <input type="radio" id="" name="choose" value="asc" <?php if(isset($_SESSION['checkTheAsc'])) echo "checked='checked'"; ?>>
                                <!-- label tp display the instructive text -->
                                <label for="friends">Asc (A-Z)</label>
                                <br>
                                <!-- input to allow the user to interact -->
                                <input type="radio" id="" name="choose" value="desc" <?php if(isset($_SESSION['checkTheDesc'])) echo "checked='checked'"; ?>>
                                <!-- label tp display the instructive text -->
                                <label for="photo">Desc (Z-A)</label>
                                <br>
                                <!-- input to allow the user to interact -->
                                <input type="checkbox" id="" name="online" value="Online" <?php if(isset($_SESSION['checkTheBox'])) echo "checked='checked'"; ?>>
                                <!-- label tp display the instructive text -->
                        ...
                        <a href="../loggedInPage.php"><button class="home">Home</button></a>
                    </div>
                            ...
                            foreach ($_SESSION['outcome'] as $row) // loop inside the variable to retrieve each data
                            {
                                if (!empty($row->image)) // if user has photo 
                                {
                                    ...
                                    '<div class="row border rounder m-3 justify-content-around  text-center overflow-hidden fontSize">'.
                                        '<div class="col-lg-2 text-nowrap text-truncate">'.'<a class="clickUser" href="../Models/Core.php?user_ID_details='.$row->user_ID.' ">' . $row->user_ID . '</a>'.'</div>' .
                                        '<div class="col-lg-2 text-nowrap text-truncate">'.'<a class="clickUser" href="../Models/Core.php?user_ID_details='.$row->user_ID.' ">' . $row->email_address . '</a>'.'</div>' .
